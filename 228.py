...
def skin(callback_query: types.CallbackQuery):
    if callback_query.data == 'skins':
        markup=types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton(text = 'Arcana на Crystal Maiden', callback_data='cm'))
        markup.add(types.InlineKeyboardButton(text = 'Arcana на Pudge', callback_data='pud'))
        markup.add(types.InlineKeyboardButton(text = 'Arcana на Shadow Fiend', callback_data='sf'))
        markup.add(types.InlineKeyboardButton(text = 'Arcana на Phantom Assasin', callback_data='pha'))
...
@bot.callback_query_handler(func=lambda callback_query: callback_query.data == 'cm')
def hero_info(callback_query: types.CallbackQuery):
     if callback_query.data == 'cm':
        uesr_id=callback_query.message.from_user.id
        items_id = callback_query.data.split("_")[0]
        connect = sqlite3.connect(config.get('DB_NAME'))
        curs = connect.cursor()
        ...
        bot.send_message(callback_query.message.chat.id, f'Вот описание героя:{items_id}')
